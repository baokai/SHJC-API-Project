...
namespace Learun.Application.Organization
{
    ...
    public class CompanyService : RepositoryFactory
    {
        ...
        /// </summary>
        /// <returns></returns>
        public CompanyEntity GetBillingUnitName(string code)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append("SELECT ");
                strSql.Append(fieldSql);
                strSql.Append(" FROM LR_Base_Company t WHERE t.F_EnabledMark = 1 AND t.F_SortCode='"+code+"'AND t.F_DeleteMark = 0  ORDER BY t.F_ParentId,t.F_FullName ");
                return this.BaseRepository().FindList<CompanyEntity>(strSql.ToString()).FirstOrDefault();
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
