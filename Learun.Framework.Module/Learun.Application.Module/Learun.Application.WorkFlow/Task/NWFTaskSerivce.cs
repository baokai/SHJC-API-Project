...
namespace Learun.Application.WorkFlow
{
    ...
    public class NWFTaskSerivce : RepositoryFactory
    {
        ...
        public void CommentsSave(string WorkFlowId, string id,string CommentsName)
        {
            try
            {
                ...
                entity.nodId = id;
                entity.CommentsName = CommentsName;
                entity.Create();
                var user = this.BaseRepository().FindEntity<UserEntity>(entity.CreateUser);
                entity.CreateUserName = user.F_RealName;
        ...
        public void CommentsUpdate(string id, CommentsEntity strEntity)
        {
            try
            {
                strEntity.Modify(id);
                this.BaseRepository().Update(strEntity );
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public NWFTaskEntity GetEntityForProcessId(string processid)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM adms706.dbo.lr_nwf_task WHERE F_ProcessId =@processid");
                return this.BaseRepository().FindEntity<NWFTaskEntity>(strSql.ToString(), new { processid });
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public NWFTaskEntity GetEntityF_PrevNodeIdbyProcessId(string processid,string prevNodeId)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM adms706.dbo.lr_nwf_task WHERE F_ProcessId =@processid and F_PrevNodeId=@prevNodeId");
                return this.BaseRepository().FindEntity<NWFTaskEntity>(strSql.ToString(), new { processid, prevNodeId });
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        /// <returns></returns>
        public NWFTaskEntity GetEntityChildProcessId( string processId)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM LR_NWF_Task WHERE F_ChildProcessId = @processId");
                return this.BaseRepository().FindEntity<NWFTaskEntity>(strSql.ToString(), new { processId});
            }
            catch (Exception ex)
            {
        ...
        /// <param name="taskId">任务主键</param>
        /// <returns></returns>
        public IEnumerable<NWFTaskRelationEntity> GetTaskUserList_NodbWhere(string taskId)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM LR_NWF_TaskRelation WHERE F_TaskId = '"+ taskId +"'  ORDER BY F_Sort");
                return this.BaseRepository().FindList_NodbWhere<NWFTaskRelationEntity>(strSql.ToString());
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
