    ...
    public class NWFProcessSericve : RepositoryFactory
    {
        ...
        public IEnumerable<NWFProcessEntity> GetMyPageList(string userId, Pagination pagination, string queryJson, string schemeCode)
        {
            try
            {
                ...
                var queryParam = queryJson.ToJObject();
                //去除管理员的id
                if (userId != "1e5dfa6a-6f0c-454c-b1ac-aeafef95aea5")
                {
        ...
        public IEnumerable<NWFProcessEntity> GetMyTaskPageList(UserInfo userInfo, Pagination pagination, string queryJson, string schemeCode, bool isBatchAudit = false)
        {
            try
            {
                ...
                strSql.Append(@"SELECT
                                    ...
                                    p.F_CreateUserName,
                                    p.F_IsStart,
ISNULL('金额:'+CONVERT(nvarchar(50),CONVERT(decimal(18,2),py.PaymentAmount)),p.F_Title) as F_Title
                                FROM
	                                (
		                                SELECT
			                                F_TaskId
                ...
                strSql.Append(@") GROUP BY
                                ...
                                left join learunOAWFForm.dbo.ProjectContract pc on pc.WorkFlowId=p.F_Id
                                left join learunOAWFForm.dbo.ProjectBilling pb on pb.WorkFlowId=p.F_Id
                                left join learunOAWFForm.dbo.ProjectPayment pr on pr.WorkFlowId=p.F_Id 

                                left join learunOAWFForm.dbo.ProjectTask pt on pt.WorkFlowId=p.F_Id 
                                left join learunOAWFForm.dbo.Payment py on py.WorkFlowId=p.F_Id   
                                left join learunOAWFForm.dbo.Project a on a.Id = pc.ProjectId
                                WHERE
                ...
                if (!queryParam["keyword"].IsEmpty())
                {
                    ...
                    strSql.Append(" AND ( p.F_Title like @keyword OR  p.F_SchemeName like @keyword OR a.CustName like @keyword OR p.F_CreateUserName like @keyword ) ");
                }
                ...
                pagination.sidx = " F_IsUrge DESC, " + pagination.sidx + " DESC ";
        ...
        public IEnumerable<CommentsVo> GetCommentsList(string WorkFlowId)
        {
            ...
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM  lr_base_comments where WorkFlowId='" + WorkFlowId + "' order by CreateTime desc ");
                return this.BaseRepository().FindList<CommentsVo>(strSql.ToString());
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public CommentsVo GetCommentsListWorkFlowId(string WorkFlowId)
        {
            try
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM  lr_base_comments where WorkFlowId='" + WorkFlowId + "' order by CreateTime desc ");
                return this.BaseRepository().FindList<CommentsVo>(strSql.ToString()).FirstOrDefault();
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public IEnumerable<CommentsEntity> GetCommentsListById(string WorkFlowId, string id)
        {
            ...
            {
                var strSql = new StringBuilder();
                strSql.Append(@"SELECT * FROM  lr_base_comments where WorkFlowId='" + WorkFlowId + "' and nodId='" + id + "'ORDER BY CreateTime DESC");
                return this.BaseRepository().FindList<CommentsEntity>(strSql.ToString());
        ...
        public List<CommentsEntity> GetCommentsListByIdNode(string id)
        {
            try
            {
                ...
                return this.BaseRepository().FindList<CommentsEntity>(t => t.nodId == id).AsList();
          ...
          public IEnumerable<NWFProcessEntity> GetMyFinishTaskPageList(UserInfo userInfo, Pagination pagination, string queryJson, string schemeCode)
          {
              try
              {
                  ...
                  strSql.Append(@"SELECT
                       ...
                       left join learunOAWFForm.dbo.ProjectContract pc on pc.WorkFlowId=p.F_Id
                       left join learunOAWFForm.dbo.Project a on a.Id = pc.ProjectId   where 1=1 
                   ");
                  var queryParam = queryJson.ToJObject();
                  DateTime startTime = DateTime.Now, endTime = DateTime.Now;
...
left join learunOAWFForm.dbo.Project a on a.Id = pc.ProjectId   where 1 = 1*/
        ...
        /// <param name="taskMsgList">流程消息列表</param>
        /// <param name="taskLogEntity">任务日志</param>
        public void SaveTask(List<NWFTaskEntity> taskList, List<NWFTaskMsgEntity> taskMsgList, NWFTaskLogEntity taskLogEntity)
        {
        ...
        /// <returns></returns>
        public NWFProcessEntity GetTitle(string processId)
        {
            try
            {
                return this.BaseRepository().FindEntity<NWFProcessEntity>(processId);
            }
            catch (Exception ex)
            {
