...
namespace Learun.Application.WorkFlow
{
    ...
    public class NWFProcessBLL : NWFProcessIBLL
    {
        ...
        /// <returns></returns>
        public IEnumerable<CommentsVo> GetCommentsList(string WorkFlowId)
        {
            return nWFProcessSerive.GetCommentsList(WorkFlowId);
        }
        public CommentsVo GetCommentsListWorkFlowId(string WorkFlowId)
        {
            return nWFProcessSerive.GetCommentsListWorkFlowId(WorkFlowId);
        }
        public IEnumerable<CommentsEntity> GetCommentsListById(string WorkFlowId, string id)
        {
            return nWFProcessSerive.GetCommentsListById(WorkFlowId, id);
        }
        /// <summary>
        /// 根据节点查询对应数据
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public List<CommentsEntity> GetCommentsListByIdNode(string id)
        {
            return nWFProcessSerive.GetCommentsListByIdNode(id);
        }
        /// <summary>
        /// 获取未处理任务列表
        /// </summary>
        ...
        public void GetEntityByWorkFlowId(string keyValue, NWFProcessEntity entity)
        {
            nWFProcessSerive.GetEntityByWorkFlowId(keyValue, entity);
        }
        #endregion
                    ...
                    foreach (var item in task.nWFUserInfoList)
                    {
                        ...
                        switch (task.F_Type)
                        {
                            ...
                            case 2:// 查阅节点
                                ...
                                break;
                            case 3:// 转交
                                nWFTaskMsgEntity.F_Content += "需要审核，来自转交，发起人" + paramConfig.CreateUser.Name + "。";
                                break;
                            case 4:// 子流程节点
        ...
        public void CreateFlow(string schemeCode, string processId, string title, int level, string auditors, UserInfo userInfo)
        {
            if (title == "" || title == null)
            {
                var data = nWFProcessSerive.GetTitle(processId);
                if (data.F_Title != null)
                {
                    title = data.F_Title;
                }
        ...
        /// <param name="processId">流程进程主键</param>
        /// <param name="userInfo">当前操作人信息</param>
        public void AgainCreateFlow(string processId, UserInfo userInfo, string des)
        {
            // 初始化流程引擎
            NWFIEngine nWFIEngine = _Bootstraper("", processId, "", userInfo);
            NWFEngineParamConfig nWFEngineParamConfig = nWFIEngine.GetConfig();
            ...
            List<NWFNodeInfo> list1 = nWFIEngine.GetNextTaskNode(nodeInfo, "agree", false, lineList1);
            // 创建任务
            List<NWFTaskEntity> taskList1 = _CreateTask(list1, nodeInfo, nWFEngineParamConfig);
            // 创建任务消息
            List<NWFTaskMsgEntity> taskMsgList1 = _CreateTaskMsg(taskList1, nWFEngineParamConfig);
            // 创建任务日志信息
            //NWFTaskLogEntity nWFTaskLogEntity1 = new NWFTaskLogEntity()
            //{
            //    F_ProcessId = processId,
            //    F_OperationCode = "create",
            //    F_OperationName = "重新创建流程",
            //    F_NodeId = nodeInfo.id,
            //    F_NodeName = nodeInfo.name,
            //    F_TaskType = 0,
            //    F_CreateUserId = userInfo.userId,
            ...
            //// 保存信息
            //nWFProcessSerive.SaveTask( taskList1, taskMsgList1, nWFTaskLogEntity1);
            // 获取任务实体
        ...
        public void AuditFlowstest(string operationCode, string taskIds, UserInfo userInfo)
        {
            string[] taskIdList = taskIds.Split(',');
            foreach (var taskId in taskIdList)
            {
        ...
        /// <param name="processId">流程进程主键</param>
        /// <param name="taskId">流程任务主键</param>
        /// <param name="userId">转交人员</param>
        /// <param name="des">转交说明</param>
        /// <param name="userInfo">当前操作人信息</param>
        public void SignFlow(string processId, string taskId, string userId, string des, UserInfo userInfo)
        {
            ...
            NWFTaskMsgEntity nWFTaskMsgEntity = new NWFTaskMsgEntity()
            {
                ...
                F_ToName = nWFEngineParamConfig.CreateUser.Name,
                F_Title = nWFEngineParamConfig.SchemeName,
                F_Content = "你的流程有状态的更新：" + nWFEngineParamConfig.CurrentUser.Name + "转交",
                NodeId = nWFIEngine.GetStartNode().id
            };
            nWFTaskMsgEntity.Create();
            ...
            if (nWFEngineParamConfig.IsChild == 1)
            {
                ...
                nWFTaskMsgEntity.F_ToAccount = pNWFEngineParamConfig.CreateUser.Account;
                nWFTaskMsgEntity.F_Title = pNWFEngineParamConfig.SchemeName;
                nWFTaskMsgEntity.F_Content = "你的流程【子流程:" + nWFEngineParamConfig.SchemeName + "】有状态的更新：" + nWFEngineParamConfig.CurrentUser.Name + "转交";
                nWFTaskMsgEntity.NodeId = pNWFIEngine.GetStartNode().id;
        ...
        public void SignAuditFlow(string operationCode, string processId, string taskId, string des, UserInfo userInfo)
        {
            ...
            NWFTaskLogEntity nWFTaskLogEntity = new NWFTaskLogEntity()
            {
                F_ProcessId = processId,
                F_OperationCode = operationCode,
                F_OperationName = "【转交】" + (operationCode == "agree" ? "同意" : "不同意"),
                F_NodeId = taskEntiy.F_NodeId,
                F_NodeName = taskEntiy.F_NodeName,
                F_PrevNodeId = taskEntiy.F_PrevNodeId,
                F_PrevNodeName = taskEntiy.F_PrevNodeName,
            ...
            NWFTaskMsgEntity nWFTaskMsgEntity = new NWFTaskMsgEntity()
            {
                ...
                F_ToName = nWFEngineParamConfig.CreateUser.Name,
                F_Title = nWFEngineParamConfig.SchemeName,
                F_Content = "你的流程有状态的更新：" + nWFEngineParamConfig.CurrentUser.Name + "【转交】" + (operationCode == "agree" ? "同意" : "不同意"),
                NodeId = nWFIEngine.GetStartNode().id
            };
            nWFTaskMsgEntity.Create();
        ...
        /// <param name="processId">流程进程主键</param>
        /// <param name="userInfo">当前操作人信息</param>
        public int? BoolRevokeFlow(string processId, UserInfo userInfo)
        {
            NWFProcessEntity processEntity = nWFProcessSerive.GetEntity(processId);
            return processEntity.F_IsStart;
        }
        ...
        public IEnumerable<NWFTaskEntity> GetTaskUserList_NodbWhere(string processId)
        {
            if (string.IsNullOrEmpty(processId))
            {
                return new List<NWFTaskEntity>();
            ...
            foreach (var item in taskList)
            {
                item.nWFUserInfoList = new List<NWFUserInfo>();
                // 获取当前任务执行人
                List<NWFTaskRelationEntity> taskUserList = (List<NWFTaskRelationEntity>)nWFTaskIBLL.GetTaskUserList_NodbWhere(item.F_Id);
                foreach (var user in taskUserList)
                {
                    if (user.F_Result == 0 && user.F_Mark == 0)
                    {
        ...
        public void deleteTaskList(IEnumerable<NWFTaskEntity> taskList, string processId)
        {
            nWFProcessSerive.deleteTaskList(taskList, processId);
        }
