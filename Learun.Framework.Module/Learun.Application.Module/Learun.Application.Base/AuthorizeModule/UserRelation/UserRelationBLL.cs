...
namespace Learun.Application.Base.AuthorizeModule
{
    ...
    public class UserRelationBLL : UserRelationIBLL
    {
        ...
        /// 分类:1-角色2-岗位</param>
        /// <returns></returns>
        public List<UserRelationEntity> GetUserRoleList(string userId)
        {
            try
            {
                var list = (List<UserRelationEntity>)userRelationService.GetUserRoleList(userId);
                return list;
            }
            catch (Exception ex)
            {
        ...
        public UserRelationEntity GetUserId(string userId)
        {
            try
            {
                return userRelationService.GetUserId(userId);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
