...
namespace Learun.Application.Base.SystemModule
{
    ...
    public class DataItemBLL : DataItemIBLL
    {
        ...
        /// <param name="itemCode">分类编号</param>
        /// <returns></returns>
        public List<TreeModel> GetDetailTreeHZ(string itemCode)
        {
            try
            {
                List<DataItemDetailEntity> list = GetDetailList(itemCode);
                ...
                foreach (var item in list)
                {
                    var user = LoginUserInfo.Get().departmentId;
                    var data = departmentIBLL.GetEntity(user);

                    TreeModel node = new TreeModel();
                    //if (item.F_ItemName == data.F_FullName || int.Parse(item.F_ItemValue) <= 1)
                    if (item.F_ItemName == data.F_FullName || item.F_ItemValue == "1" || item.F_ItemValue == "65")
                    {
                        //node.id = item.F_ItemDetailId;
        ...
        /// 根据id获取获取数据字典的名称
        /// </summary>
        /// <param name="itemValue">id</param>
        /// <param name="itemCode">分类编号</param>
        /// <returns></returns>
        public DataItemDetailEntity GetDetailItemName(string itemValue, string itemCode)
        {
            try
            {
                DataItemDetailEntity res = new DataItemDetailEntity();
                List<DataItemDetailEntity> list = GetDetailList(itemCode);

                res = list.FindAll(i => i.F_ItemValue == itemValue).FirstOrDefault();
                return res;
        ...
        public DataItemDetailEntity GetDetailItemName1(string itemValue, string itemCode)
        {
            try
            {
                DataItemDetailEntity res = new DataItemDetailEntity();
        ...
        /// <param name="itemCode">分类编号</param>
        /// <returns></returns>
        public DataItemDetailEntity GetDetailItemNameById(string itemValue, string itemCode)
        {
            try
            {
                DataItemDetailEntity res = new DataItemDetailEntity();
