...
(function ($, learun) {
    ...
    */
    ...
    //不会弹窗提醒保存成功
    $.lrSaveForm_2 = function (url, param, callback, isNotClosed) {
        param['__RequestVerificationToken'] = $.lrToken;
        learun.httpAsyncPost(url, param, function (res) {
            if (res.code == learun.httpCode.success) {
                if (!!callback) {
    ...
    // 下拉框
    $.fn.lrDepartmentSelectHZ = function (op) {
        ...
        var dfop = {
            ...
            url: top.$.rootUrl + '/LR_SystemModule/DataItem/GetDetailTreeHZ',
            // 访问数据接口参数
            param: { itemCode: 'ContractSubject' },
        }
        //console.log('123')
        op = op || {};
        //dfop.param.companyId = op.value;
    ...
    //合同主体选择
    $.fn.lrContractSubjectSelect = function (op) {
        // op:parentId 父级id,maxHeight 200,
        var dfop = {
            type: 'tree',
            // 是否允许搜索
            ...
            // 访问数据接口地址
            //url: top.$.rootUrl + '/LR_OrganizationModule/Department/GetTree',
            url: top.$.rootUrl + '/LR_SystemModule/DataItem/GetContractSubjectTree',
            // 访问数据接口参数
            param: { itemCode: 'ContractSubject' },
        }
        //console.log('123')
    ...
    // 多部门信息下拉框
    $.fn.lrMoreDepartmentSelect = function (op) {
        // op:parentId 父级id,maxHeight 200,
        var dfop = {
            type: 'tree',
            // 是否允许搜索
    ...
    //添加
    $.fn.lrAddQuery = function (search, height, width) {
        var $this = $(this);
        var contentHtml = $this.html();
        $this.addClass('lr-query-wrap');
        ...
        var _html = '';
        _html += '<div class="lr-query-btn"><i class="fa fa-search"></i>&nbsp;填写信息</div>';
        _html += '<div class="lr-query-content">';
        //_html += '<div class="lr-query-formcontent">';
        _html += contentHtml;
        //_html += '</div>';
        ...
        _html += '<div class="lr-query-content-bottom">';
        _html += '<a id="lr_btn_queryReset" class="btn btn-default">&nbsp;重&nbsp;&nbsp;置</a>';
        _html += '<a id="lr_btn_querySearchAdd" class="btn btn-primary">&nbsp;确&nbsp;&nbsp;认</a>';
        _html += '</div>';
        _html += '</div>';
        $this.html(_html);
        $this.find('.lr-query-formcontent').show();
        ...
        $this.find('#lr_btn_querySearchAdd').on('click', function () {
            var $content = $this.find('.lr-query-content');
            var query = $content.lrGetFormData();
            $content.removeClass('active');
            search(query);
