...
namespace Learun.Application.Web.Areas.LR_AuthorizeModule.Controllers
{
    ...
    public class UserRelationController : MvcControllerBase
    {
        ...
        public ActionResult SaveForm(string objectId, int category, string userIds)
        {
            ...
            if (category == 1)
            {
                return Success("保存成功！", "角色用户对应对象", string.IsNullOrEmpty(objectId) ? OperationType.Create : OperationType.Update, objectId, objectId);
            }
            return Success("保存成功！", "岗位用户对应对象", string.IsNullOrEmpty(objectId) ? OperationType.Create : OperationType.Update, objectId, objectId);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        [AjaxOnly]
