...
namespace Learun.Application.Web.Areas.LR_AuthorizeModule.Controllers
{
    ...
    public class AuthorizeController : MvcControllerBase
    {
        ...
        public ActionResult SaveForm(string objectId, int objectType, string strModuleId, string strModuleButtonId, string strModuleColumnId, string strModuleFormId)
        {
            ...
            if (objectType == 1)
            {
                return Success("保存成功！", "功能权限", string.IsNullOrEmpty(objectId) ? OperationType.Create : OperationType.Update, objectId, "角色权限，" + objectId + ",功能Id," + strModuleId);
            }
            return Success("保存成功！", "功能权限", string.IsNullOrEmpty(objectId) ? OperationType.Create : OperationType.Update, objectId, "用户权限，" + objectId + ",功能Id," + strModuleId);
        }
        /// <summary>
        /// 保存表单数据
        /// </summary>
