...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#F_File').lrUploader();
            $('#LR_ERP_SalesOfferDetail').jfGrid({
                headData: [
                    {
                        ...
                        ,edit:{
                            ...
                            change: function (data, rownum, selectData) {
                                ...
                                $('#LR_ERP_SalesOfferDetail').jfGridSet('updateRow', rownum);
                            },
                            op: {
                                width: 600,
                                height: 400,
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/ERPDemo/SalesOffer/GetFormData?keyValue=' + keyValue, function (data) {
                    for (var id in data) {
    ...
    acceptClick = function (callBack) {
        ...
        var postData = {};
        postData.strEntity = JSON.stringify($('[data-table="LR_ERP_SalesOffer"]').lrGetFormData());
        postData.strlR_ERP_SalesOfferDetailList = JSON.stringify($('#LR_ERP_SalesOfferDetail').jfGridGet('rowdatas'));
        $.lrSaveForm(top.$.rootUrl + '/ERPDemo/SalesOffer/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
