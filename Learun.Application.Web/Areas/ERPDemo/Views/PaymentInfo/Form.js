...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            $('#F_PurchaseType').lrDataItemSelect({ code: 'Client_PaymentMode' });
            ...
            $('#LR_ERP_PaymentInfoDetail').jfGrid({
                headData: [
                    {
                        ...
                        ,edit:{
                            ...
                            change: function (data, rownum, selectData) {
                                ...
                                $('#LR_ERP_PaymentInfoDetail').jfGridSet('updateRow', rownum);
                            },
                            op: {
                                width: 600,
                                height: 400,
        ...
        initData: function () {
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/ERPDemo/PaymentInfo/GetFormData?keyValue=' + keyValue, function (data) {
                    for (var id in data) {
    ...
    acceptClick = function (callBack) {
        ...
        var postData = {};
        postData.strEntity = JSON.stringify($('[data-table="LR_ERP_PaymentInfo"]').lrGetFormData());
        postData.strlR_ERP_PaymentInfoDetailList = JSON.stringify($('#LR_ERP_PaymentInfoDetail').jfGridGet('rowdatas'));
        $.lrSaveForm(top.$.rootUrl + '/ERPDemo/PaymentInfo/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
