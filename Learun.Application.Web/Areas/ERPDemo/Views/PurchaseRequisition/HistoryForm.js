...
var bootstrap = function ($, learun) {
    ...
    var page = {
                ...
                if (learun.checkrow(purchaseId)) {
                    if (purchaseId != nowpurchaseId) {
                        learun.layerConfirm('是否要更新到该版本！', function (res) {
                            if (res) {
                                learun.postForm(top.$.rootUrl + '/ERPDemo/PurchaseRequisition/Update', { PurchaseInfoId: keyValue, purchaseId: purchaseId }, function () {
        ...
        initGrid: function () {
            $('#gridtable').jfGrid({
                url: top.$.rootUrl + '/ERPDemo/PurchaseRequisition/GetPurchasePageList',
                headData: [
                    { label: "单据", name: "F_PurchaseNo", width: 120, align: "left" },
                    {
