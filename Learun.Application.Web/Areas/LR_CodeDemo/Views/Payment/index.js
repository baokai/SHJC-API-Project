    ...
    var page = {
        ...
        bind: function () {
          ...
          /*  $('#ProjectId').lrformselect({
            ...
            $('#lr-export').on('click', function () {
                learun.loading(true);
                var keyword = $('#txt_Keyword').val();
                $.QueryJson = $('#multiple_condition_query').lrGetFormData();
                $.QueryJson = $.QueryJson || {};             
                $.QueryJson.StartTime = startTime;
                $.QueryJson.EndTime = endTime;
                $.QueryJson.keyword = keyword;
                //$.QueryJson.Payee = $('#Payee').val();
                //$.QueryJson.PayType = $('#PayType').val();
                //$.QueryJson.ContractSubmitter = $('#ContractSubmitter').val();
                //$.QueryJson.PaymentReason = $('#PaymentReason').val();
                //$.QueryJson.PaymentAmount = $('#PaymentAmount').val();

                learun.httpAsync('Get', top.$.rootUrl + '/LR_CodeDemo/Payment/GetPageListAll', { queryJson: JSON.stringify($.QueryJson) }, function (data) {
                    var uuid = data.rows;
                    learun.download({
                        method: "POST",
        ...
        initGird: function () {
            $('#gridtable').lrAuthorizeJfGrid({
                url: top.$.rootUrl + '/LR_CodeDemo/Payment/GetPageList',
                headData: [
                    {
                    ...
                    {
                        label: "提审人", name: "ContractSubmitter", width: 90, align: "center",
                        formatterAsync: function (callback, value, row) {
                    ...
                    {
                        label: "创建人", name: "CreateUser", width: 80, align: "left",
                        formatterAsync: function (callback, value, row) {
                            learun.clientdata.getAsync('user', {
                                key: value,
                                callback: function (item) {
...
/*            param = param || {};
            ...
            var keyword = $('#txt_Keyword').val();
            param = $('#multiple_condition_query').lrGetFormData();
            param = param || {};
            param.StartTime = startTime;
            param.EndTime = endTime;
