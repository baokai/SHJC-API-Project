...
var ContractStatus = request('ContractStatus');
var ContractNo = request('ContractNo');
var pr = request('WorkFlowId');
var contract = 1;
var contractNotext = "";
var a=""
// 设置权限
...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#ContractSubject').lrContractSubjectSelect();
         ...
         /*   $('#lr-contract').on('click', function () {
            ...
            $('#ContractType').on('click', function () {
                var formData = $('body').lrGetFormData();
                //alert(formData.ContractType);
                if (formData.ContractType != ContractType && ContractNo == "") {
                    if (formData.ContractType == 1) {
          ...
          /*  window.onload = function () {
            ...
            $('#ProjectId').lrformselect({
                ...
                select: function (item) {   
                    ...
                    if (item.ContractNo != null) {
                        alert("该项目合同编号已存在，可以选择增补按钮");
                        contractNotext = item.ContractNo;
                    }
            ...
            if ((ContractStatus != 1 && keyValue != "") & (ContractStatus != 11)) {
                $('#ContractFile').lrUploader({ isDown: true, isView: true, isUpload: false });
            } else { 
    ...
    save = function (processId, callBack, i) {
        ...
        $.lrSaveForm(top.$.rootUrl + '/LR_CodeDemo/ProjectContract/SaveForm?keyValue=' + keyValue + "&autoFlag=" + autoFlag, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack(res, i);
            }
