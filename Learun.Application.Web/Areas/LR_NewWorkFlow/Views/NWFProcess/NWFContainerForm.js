...
var Title = request('F_Title');
var plList = [];
function getNodePLlist() {
    $.ajax({
        type: 'POST',
        ...
        success: function (res) {
            ...
            plList = res.data.nodes;
            console.log('123', plList)
        }
    })
    return plList
}
...
var bootstrap = function ($, learun) {
    //评论查看   
    $('#lr_content').on('click', function () {

        if (processId != null && processId != "") {
            learun.frameTab.open({ F_ModuleId: 'order_add', F_Icon: 'fa fa-file-text-o', F_FullName: '评论信息', F_UrlAddress: '/LR_NewWorkFlow/NWFProcess/ComsIndex?processId=' + processId });
        }
    ...
    nwflow = {
                ...
                callback: function (departmentMap) {
                    learun.clientdata.getAllAsync('user', {
                        callback: function (userMap) {
                            ...
                            $('#auditinfo').lrtimeline(nodelist, plINP, showFlowComment, plList);
                        }
                    });
                }
            });
        ...
        plINP1: function (e) {
            ...
            //document.getElementById("plInp").style.display = "block";
            learun.layerForm({
                id: 'form',
                title: '评论',
                url: top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/LookFlowForm',
                width: 800,
                height: 600,
                callBack: function (id) {
                    var res = false;
        ...
        bind: function () {
            ...
            $('#comments').on('click', function () {
                learun.layerForm({
                    id: 'form',
                    title: '流程评论',
                    url: top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/CommentsFrom?processId=' + nwflow.pProcessId + '&taskId=' + nwflow.pTaskId,
                    width: 800,
                    height: 400,
                    callBack: function (id) {
                        var res = false;
    ...
    function plINP(e) {
        learun.layerForm({
            ...
            title: '流程评论',
            url: top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/CreateCommentForm?processId=' + nwflow.processId + '&taskId=' + e,
            width: 500,
            height: 300,
            callBack: function (id) {
                var res = false;
    ...
    function showFlowComment(data) {
        learun.layerForm({
            id: 'FlowComment',
            title: '流程评论记录',
            url: top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/FlowComment?processId=' + nwflow.processId + '&taskId=' + data,
            width: 500,
            height: 300,
            btn: null
        });
