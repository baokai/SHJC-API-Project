        ...
        initGrid: function () {
            $('#gridtable').lrAuthorizeJfGrid({
                ...
                headData: [
                                    ...
                                    $cell.on('click', '.revoke', function () {// 删除
                                        learun.layerConfirm('是否确认撤销流程？', function (res, _index) {
                                            if (res) {
                                               ...
                                               $.ajax({
                                                    url: top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/BoolRevokeFlow',
                                                    type: 'POST',
                                                    data: postData,
                                                    dataType: "json",
                                                   success: function (res) {
                                                        if (res == 0) {
                                                            ...
                                                            learun.alert.error("此流程已受理")
                                                        }
                                                    },
                                                })
                                                /*learun.httpAsync('Post', top.$.rootUrl + '/LR_NewWorkFlow/NWFProcess/BoolRevokeFlow', postData, function (data) {
                                                    console.log(data)
                                                });*/
                                                /*learun.loading(true, '撤销流程...');
                                                var postData = {
                    ...
                    {  label: "部门", name: "DepartmentName", width: 70, align: "center" },
                ...
                onSelectRow: function (row) {
                    if (categoryId == '2') {
                        if (row.F_TaskType == 5) {
                            ...
                            $('#lr_verify span').text("【转交】" + row.F_TaskName);
                        }
                        else {
        ...
        verify: function () {
            ...
            switch (taskType) {
                ...
                case 2:// 传阅
                    ...
                    break;
                case 3:// 加签
                    learun.frameTab.open({ F_ModuleId: taskId, F_Icon: 'fa magic', F_FullName: '转交审核-' + title + '/' + taskName, F_UrlAddress: '/LR_NewWorkFlow/NWFProcess/NWFContainerForm?tabIframeId=' + taskId + '&type=signAudit' + "&processId=" + processId + "&taskId=" + taskId });
                    break;
                case 4:// 子流程
