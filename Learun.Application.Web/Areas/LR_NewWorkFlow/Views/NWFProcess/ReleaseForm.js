...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#createUser').lrUserSelect(0, function (item) {
                learun.httpAsync('GET', top.$.rootUrl + '/LR_NewWorkFlow/NWFScheme/GetMyInfoList', {userId:item.value}, function (data) {
                    schemeList = data;
                    render();
                });
            });
            var loginInfo = learun.clientdata.get(['userinfo']);
            $('#createUser').lrformselectSet(loginInfo.userId);
        }
    };
    // 保存数据
    acceptClick = function () {
        ...
        if (!!shcemeCode) {
            ...
            learun.frameTab.open({ F_ModuleId: id, F_Icon: 'fa magic', F_FullName: '发起流程【' + $selected[0].shceme.F_Name + '】', F_UrlAddress: '/LR_NewWorkFlow/NWFProcess/NWFContainerForm?shcemeCode=' + shcemeCode + '&tabIframeId=' + id + '&type=create' + "&createUserId=" + createUser });
            return true;
        }
        else {
