...
var bootstrap = function ($, learun) {
    ...
    var page = {
                ...
                if (learun.checkrow(schemeId)) {
                    if (schemeId != nowschemeId) {
                        learun.layerConfirm('是否要更新到该版本！', function (res) {
                            if (res) {
                                learun.postForm(top.$.rootUrl + '/LR_NewWorkFlow/NWFScheme/UpdateScheme', { schemeInfoId: keyValue, schemeId: schemeId }, function () {
        ...
        initGrid: function () {
            $('#gridtable').jfGrid({
                url: top.$.rootUrl + '/LR_NewWorkFlow/NWFScheme/GetSchemePageList',
                headData: [
