...
namespace Learun.Application.Web.Areas.LR_SystemModule.Controllers
{
    ...
    public class DataItemController : MvcControllerBase
    {
        ...
        /// <summary>
        /// 获取数据字典明显树形数据合作伙伴合同主体
        /// </summary>
        /// <param name="itemCode">分类编号</param>
        /// <returns></returns>
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetDetailTreeHZ(string itemCode)
        {
            var data = dataItemIBLL.GetDetailTreeHZ(itemCode);
        ...
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetContractSubjectTree(string itemCode)
        {
            var data = dataItemIBLL.GetDetailTree(itemCode);
