...
namespace Learun.Application.Web.Controllers
{
    ...
    public class LoginController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult CheckLoginFromJava(string usercode)
        {
            UserEntity userEntity = userBll.CheckLoginFromJava(usercode);
            #region 写入日志
            LogEntity logEntity = new LogEntity();
            logEntity.F_CategoryId = 1;
            logEntity.F_OperateTypeId = ((int)OperationType.Login).ToString();
            logEntity.F_OperateType = EnumAttribute.GetDescription(OperationType.Login);
            logEntity.F_OperateAccount = usercode + "(" + userEntity.F_RealName + ")";
            logEntity.F_OperateUserId = !string.IsNullOrEmpty(userEntity.F_UserId) ? userEntity.F_UserId : usercode;
            logEntity.F_Module = Config.GetValue("SoftName");
            #endregion
            if (!userEntity.LoginOk)//登录失败
            {
            ...
            {
                string token = OperatorHelper.Instance.AddLoginUser(userEntity.F_Account, "Learun_ADMS_6.1_PC", null);//写入缓存信息
                //写入日志
                logEntity.F_ExecuteResult = 1;
                logEntity.F_ExecuteResultJson = "登录成功";
                logEntity.WriteLog();
